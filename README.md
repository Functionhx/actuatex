...
| **Isaac Lab + Isaac Sim / PhysX 5** | 当前 NVIDIA 机器人工作流与复杂场景 | Sim 6 原生训练、RTX/ROS 传感器、Gym/MuJoCo 双向迁移 | Lab 是训练框架，Sim 是运行时与模拟器 |
...
安装独立的 Isaac Sim `6.0.1 GA`，并固定官方适配它的 Isaac Lab 标签（该 Lab 标签仍是 beta）：
...
本机 RTX 4070 Ti SUPER 上，8192 环境实测约 15.3–15.7 万 steps/s，峰值显存 15092 MiB（92.2%）。版本说明、热启动参数、旧 Gym 策略迁移和录制命令见 [Isaac Lab / Isaac Sim 后端指南](./backends/isaac_lab/README.md)；6.0.1 的训练结果与完整验收见 [迁移报告](./docs/ISAAC_SIM_6_MIGRATION.zh-CN.md)。
...
这套仓库可以直接拆成 8 次实验。每一章都有一个明确问题、一个应读入口和一个可提交产物。
...
| **7 · 控制方法擂台** | PID、LQG、MPC、LQR 和 RL 如何公平比较？ | [`classical_control.py`](./tasks/inverted_pendulum/classical_control.py)、[完整实验报告](./docs/INVERTED_PENDULUM_BENCHMARK.zh-CN.md)、[方法全景](./docs/INVERTED_PENDULUM_CONTROL_METHODS.zh-CN.md) | 1/2/3 阶倒立摆统一指标 + 方法适用性分析 |
...
| Sim 6 通用鲁棒微调 | **两种子 RMSE 0.09445 · 提升 1.04%** | 提升小但留出种子同方向，重置未恶化 |
| Sim 6 台阶鲁棒微调 | **212/256 clean · 236/256 first-attempt** | 最终 `model10`；强随机条件，不是展示口径 |
| Sim 6 串联式轮腿，1024-env holdout | **7 falls · RMSE 0.09817** | 相比基线 23 falls，跌倒减少 69.6% |
| 1/2/3 阶倒立摆原生 PPO | **MuJoCo 100/99.98/40.87% · Isaac 100/100/49.51%** | 另含 19+ 类控制/估计方法、鲁棒矩阵与 TVLQR 起摆；[完整报告](./docs/INVERTED_PENDULUM_BENCHMARK.zh-CN.md) |

最终 Sim 6 台阶 PPT 视频生成在 `artifacts/isaac_sim_6/videos/TinyMal_IsaacSim_6_FinalPolicy_Stairs_PPT.mp4`。二进制产物默认不进入 Git；本地完整参数、哈希和评估索引见 `artifacts/isaac_sim_6/manifest.json`。
...
完整设计见 [架构说明](./docs/ARCHITECTURE.md) 与 [工业化全栈方案](./docs/INDUSTRIAL_RL_STACK.zh-CN.md)，适合答辩或代码评审的版本见 [代码修改报告](./docs/CODE_CHANGES_REPORT.zh-CN.md)。
...
├── navigation/          # ROS 2 Nav2 接口、消息与传感器配置
├── tools/
...
- Isaac Gym / Isaac Lab 从 `1024` 或 `2048` 个环境起步，观察 steps/s 与显存，再逐档提高。本机 Sim 6 纯物理训练的实测甜点为 `8192` 环境；不要把这个数字照搬到相机/雷达任务。
- “显存占用 100%”不是优化目标。更重要的是 GPU 利用率、仿真吞吐、PPO 更新时间和是否发生 OOM/系统交换。
- 大规模接触任务还要扩展 PhysX found/lost-pair buffer；本项目会按环境数自动取安全的 2 次幂容量。
...
- [x] Isaac Sim 6.0.1 GA 迁移、8192 环境微调、双种子复评与楼梯成片
...
- [ ] 串联式双轮腿：MuJoCo sim2sim、VMC 对照和复杂地形课程
- [ ] 加入历史观测策略、特权信息教师与在线适应
- [ ] 完成 G1 29-DOF Isaac/MuJoCo 原生训练、双向 sim2sim 与 ONNX/C++ 回放
