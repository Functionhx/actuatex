...
/path/to/isaac-sim-6.0.1/python.sh \
  ...
  --learning_starts 128 --batch_size 64 --device cuda:0
```
...
| Isaac SAC / TD3 smoke | 各 16 env、1024 transitions、57 GPU updates | 两种 off-policy 算法可在同一任务上更新 |
| MuJoCo PPO smoke | 8 env × 1 iteration | MuJoCo PPO 与 GPU 网络更新闭合 |
| MuJoCo SAC / TD3 smoke | 各 8 env、1024 transitions、113 GPU updates | MuJoCo 采样与共享 off-policy 实现闭合 |
...
1. 从 4096 环境短基准开始寻找 RTX 4070 Ti SUPER 的吞吐/显存甜点，再启动 PPO、SAC、TD3 正式长训；
